...
pub async fn identify_second_pass(
    ...
    let ra_half_width = (radius_deg / center_dec_deg.to_radians().cos().abs().max(0.1)).min(30.0);
    let params = vec![
    ...
    let payload: IdentificationResponse =
        serde_json::from_str(&body).map_err(|error| format!("invalid JPL response: {error}"))?;

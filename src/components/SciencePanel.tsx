  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      ...
      >
        <header className="flex items-center justify-between border-b border-sky-hairline px-5 py-4">
          <div className="flex min-w-0 items-center">
            <div className="min-w-0">
              <div id="science-panel-title" className="text-body-sm font-medium text-sky-ink">
                可疑目标列表
              </div>
              <div className="mt-0.5 text-label text-sky-mute">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4 text-label">
          {p.measurements.length === 0 ? (
            <div className="grid h-full min-h-52 place-items-center rounded-md border border-dashed border-sky-hairline-strong bg-sky-canvas-soft/30 px-6 text-center">
              <div>
                <div className="text-body-md text-sky-body">还没有可疑目标</div>
                <div className="mt-2 max-w-md text-label leading-5 text-sky-mute">
            ...
            <div className="overflow-hidden rounded-md border border-sky-hairline">
              <div className="grid grid-cols-[168px_110px_minmax(190px,1fr)_minmax(150px,0.8fr)_40px] gap-3 border-b border-sky-hairline bg-sky-canvas-soft px-3 py-2 text-caption-mono uppercase tracking-[0.12em] text-sky-mute">
                <span>目标名称</span>
                <span>测量</span>
                <span>天球坐标</span>
                <span>质量标记</span>
                  ...
                  >
                    <div>
                      <input
                        ...
                        className={`h-8 w-full rounded border bg-sky-canvas px-2 text-caption-mono text-sky-ink outline-none ${nameError ? 'border-sky-error' : 'border-sky-hairline-strong focus:border-sky-primary'}`}
                      />
                      {nameError && (
                        <div className="mt-1 text-label leading-4 text-sky-error">{nameError}</div>
                      )}
                    </div>
                    ...
                    <div className="text-caption-mono leading-5 text-sky-body">
                      <div>RA&nbsp; {measurement.ra_deg?.toFixed(7) ?? '—'}</div>
                      <div>Dec {measurement.dec_deg?.toFixed(7) ?? '—'}</div>
                    </div>
                    <div className="break-words text-caption-mono leading-5 text-sky-mute">
        ...
        <footer className="flex items-center justify-between gap-4 border-t border-sky-hairline bg-sky-canvas-soft px-5 py-3">
          <div className="text-label text-sky-mute">
